...
def run_screenshot_demo() -> None:
    ...
    output_path = output_dir / "demo_screenshot.png"
    output_path.write_bytes(image_bytes)

    logger.info(
        f"Screenshot successfully saved to {output_path.absolute()}"
    )
...
def run_typing_demo() -> None:
    ...
    logger.info(
        "Starting in 3 seconds. Please focus on a text input window..."
    )
    time.sleep(3.0)
...
def run_click_type_demo() -> None:
    ...
    logger.info(
        "Starting in 3 seconds. Please maximize your text editor..."
    )
    ...
    logger.info(
        "Executing left click to establish window focus..."
    )
    ...
    demo_text = (
        "OpenOperator automated click and type test successful."
    )
