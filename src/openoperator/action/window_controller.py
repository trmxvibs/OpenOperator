...
specific application windows by their title and detecting the currently active
window.
"""
...
class WindowController:
    ...
    def focus_window_by_title(self, title: str) -> bool:
        ...
        logger.debug(
            f"Scanning open windows for title containing: '{title}'"
        )
        ...
        if found_hwnd:
            logger.debug(
                f"Found match. HWND: {found_hwnd}. Attempting to focus..."
            )
            ...
            if success:
                logger.info(
                    f"Successfully focused window matching '{title}'."
                )
                return True

            logger.warning(
                f"Found window matching '{title}', "
                "but OS denied foreground request."
            )

            return False

        logger.warning(
            f"No visible window found matching title: '{title}'."
        )
    ...
    def get_active_window_title(self) -> Optional[str]:
        ...
        if sys.platform != "win32":
            logger.warning(
                "get_active_window_title is only supported on Windows."
            )
            return None
        ...
        try:
            ...
            logger.error(
                f"Failed to get active window title: {e}"
            )
            return None